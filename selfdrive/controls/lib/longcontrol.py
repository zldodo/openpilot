...
class LongControl():
  ...
  def update(self, active, CS, CP, long_plan, accel_limits):
    ...
    if len(long_plan.speeds) == CONTROL_N:
      v_target = interp(CP.longitudinalActuatorDelayLowerBound, T_IDXS[:CONTROL_N], long_plan.speeds)
      a_target = 2 * (v_target - long_plan.speeds[0])/CP.longitudinalActuatorDelayLowerBound - long_plan.accels[0]
      ...
      if a_target > -1.5 and long_plan.accels[0] < -2.5:
        print('HEYY')
        print(v_target, a_target, (v_target - long_plan.speeds[0])/CP.longitudinalActuatorDelayLowerBound)
        print(long_plan.accels)
        print(long_plan.speeds)
        print()
